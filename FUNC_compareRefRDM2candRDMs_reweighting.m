...
if userOptions.reweighting == true
    ...
    ylabel({'\bf RDM correlation ',['\rm[mean Spearman correlation across ', deunderscore(num2str(userOptions.boot_options.nboots)), ' bootstraps]']});
    
    % calculate and display sample SEM for each model (SD of bootstrap)
    stats_p_r.SEs = std(model_corrs);
    hold on
    errorbar(1:numel(candRDMs),y_sorted,stats_p_r.SEs(sortedIs),'Color',[0 0 0],'LineWidth',...
            2,'LineStyle','none');
    ...
    % statistical tests (currently a H=0 or 1, after a CI test)
    % TODO: convert to p values for consistency with existing plotting code
    % TODO: note that we conservatively use single correction factor across all groups of tests 
    % - probably want to change this to separate out the 3 groups of tests?
    num_tests = (2 * numel(candRDMs)) + (numel(candRDMs)*numel(candRDMs) - numel(candRDMs))/2; 
    thresh = userOptions.RDMrelatednessThreshold/num_tests; % TODO: implement alternatives to Bonferonni correction
    ...
    for m = 1:numel(candRDMs)
        for n = 1:numel(candRDMs)
            ...
            ci = quantile(m_v_n_corrs, [thresh/2, 1-(thresh/2)]);
    ...
    % calculate and store test of each model against reference RDM
    % i.e. models vs zero, and also lower noise ceiling
    ...
    for m = 1:numel(candRDMs)
        ...
        % model vs zero
        ci = quantile(model_corrs(:,m),[thresh/2, 1-(thresh/2)]);
        if ci(1) > 0
        ...
        ci = quantile(m_v_c_corrs, [thresh/2, 1-(thresh/2)]);
        if ci(1) > 0
    ...
    % TODO: add use of lower ceiling test in plotting
...
if ~isequal(userOptions.RDMrelatednessTest,'none')
    ...
    if userOptions.reweighting == true
        ...
        thresh = 0.5; % hack so that all entries == 0 will be counted as significant, and all == 1 will not
    else
...
% TODO: implement these correctly (fix ordering, and check values)
% 
